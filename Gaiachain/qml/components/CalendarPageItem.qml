...
Components.CalendarItem {
    ...
    additionalContent: Row {
        ...
        opacity: top.isCurrentMonth ? GStyle.visible : GStyle.hidden
        spacing: s(GStyle.calendarDotSize) / 3

        Items.LayoutSpacer { height: s(GStyle.calendarDotSize); width: 1 }

        Repeater {
            model: packageTypesModel

            Rectangle {
                width: visible ? s(GStyle.calendarDotSize) : 0
                height: width
                radius: width * 0.5
